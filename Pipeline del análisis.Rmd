...
```{rFunction plot PCA, include=FALSE}
library(ggplot2)
library(ggrepel)
...
```{r plotPVCA, fig.cap="Relative importance of the different factors -genotype, temperature and interaction- affecting gene expression", echo=FALSE}
#plot the results
...
for (i in colnames(cont.matrix)){
  volcanoplot(fit.main, coef=i, highlight=4, names=fit.main$ID, col="red",
...
```{rAnnotations for selected genes, echo=FALSE, results='hide', message=FALSE, error=FALSE}
...
```{rAnnotations GSEA, include=FALSE}
...
```{r tableReactop40vsRosett0, echo=FALSE}
...
```{r tableReactoRosett20vsRosett0, echo=FALSE}
...
```{r tableReactop40vsNPC0, echo=FALSE}
...
```{r tableReactoNPC20vsNPC0, echo=FALSE}
